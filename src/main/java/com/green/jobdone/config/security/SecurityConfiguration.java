...
@Configuration //빈등록 ? 스프링에서 관리하는 객체 di를 통해 관리하도록 설정하는것
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) //시큐리티가 세션을 사용하지 않는다.
                ...
                .authorizeHttpRequests(req ->
                        req.requestMatchers(HttpMethod.POST, "/api/user/sign-in", "/api/user/sign-up").permitAll() // 인증 없이 접근 허용
                                .requestMatchers("/api/admin").hasRole("ADMIN") // 관리자 권한 필요
                                .requestMatchers("/api/user/**").authenticated() // 인증 필요
